...
/*
// Version modifiée de create_text_texture pour gérer l'UTF-8
TextureAlpha* create_text_texture(const char* text, FontInfo* font, uint32_t color) {
    const unsigned char* utext = (const unsigned char*)text;
    
    // Compter d'abord le nombre réel de caractères (pas les bytes)
    int pos = 0;
    int char_count = 0;
    while (utext[pos] != 0) {
        UTF8Char ch = get_next_utf8_char(utext, &pos);
    ...
    printf("Creating texture for text: '%s' (chars: %d)\n", text, char_count);
    
    // Calculer la taille de texture nécessaire
    int tex_width = 8;
    while (tex_width < (char_count * 8)) {
        tex_width *= 2;
    }
    
    TextureAlpha* tex = create_alpha_texture(tex_width, 8);
    tex->real_width = char_count * 8;
    
    uint16_t* buffer = malloc(tex_width * 8 * 2);
    memset(buffer, 0, tex_width * 8 * 2);
    
    uint16_t color565 = 0xF800;  // Rouge pour test
    
    // Recommencer depuis le début du texte
    pos = 0;
    int char_pos = 0;
    ...
    while (utext[pos] != 0) {
        ...
        printf("Processing UTF-8 char at pos %d (code: %d, index: %d)\n", 
               char_pos, char_code, char_index);
        ...
        if(char_index >= 0) {
            for(int y = 0; y < 8; y++) {
                for(int x = 0; x < 8; x++) {
                    int bitmap_idx = (char_index * 64) + (y * 8) + x;
                    if(font->bitmap[bitmap_idx]) {
                        int buffer_idx = (y * tex_width) + (char_pos * 8) + x;
                        buffer[buffer_idx] = color565;
                    }
                }
            }
        }
    ...
    pvr_txr_load_ex(buffer, tex->texture, tex_width, 8,
                    PVR_TXRLOAD_16BPP | PVR_TXRFMT_TWIDDLED);
