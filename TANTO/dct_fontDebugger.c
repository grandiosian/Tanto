...
// Fonction simplifiée pour lire le prochain caractère UTF-8
UTF8Char get_next_utf8_char(const unsigned char* str, int* pos) {
    UTF8Char result = {{0}, 0};  // Initialisation à zéro

    // Vérification des pointeurs
    if (!str || !pos) {
        result.length = 1;  // Retourner un caractère vide mais valide
        return result;
    }

    unsigned char c = str[*pos];

    // Caractère ASCII simple (0-127)
    if (c <= 0x7F) {
        result.bytes[0] = c;
        result.length = 1;
        *pos += 1;
...
// Fonction simplifiée pour convertir UTF-8 en code de caractère
int utf8_to_extended_char(UTF8Char utf8) {
    // Si c'est un caractère ASCII simple
    if (utf8.bytes[0] <= 0x7F) {
        return utf8.bytes[0];
    }
    
    // Pour les caractères étendus, retourner directement la valeur
    return utf8.bytes[0];
}

// Fonction simplifiée pour trouver l'index du caractère
int find_char_index(int charcode) {
    // Vérification des limites
    if (charcode < 0 || charcode > 255) {
        return 0;  // Retourner l'index du premier caractère comme fallback
    }
    
    // Rechercher le caractère dans la table
    for (int i = 0; i < TOTAL_CHARS; i++) {
        if (CHAR_MAP_COMPLETE[i].charcode == charcode) {
            return i;
        }
    ...
    return 0;  // Caractère non trouvé, retourner l'index du premier caractère
}
...
dct_textureAlpha* ___create_text_texture_with_colors(const char* text, dct_fontInfo* font, uint32_t bg_color, int pwidth, int pheight) {
...
dct_textureAlpha* create_text_texture_with_colors(const char* text, dct_fontInfo* font, uint32_t bg_color, int pwidth, int pheight) {
    ...
    const unsigned char* utext = (const unsigned char*)text;
    int text_length = strlen((const char*)utext);
    uint32_t current_color = 0xFFFFFF;
    
    // Premier passage : compter les caractères et les lignes de façon sécurisée
    int pos = 0;
    int max_chars_per_line = 0;
    int current_line_length = 0;
    int num_lines = 1;
    ...
    while (pos < text_length && utext[pos] != 0) {
        ...
        // Gestion sécurisée des balises de couleur
        if (pos + 9 < text_length &&
            utext[pos] == '\\' && 
            utext[pos + 1] == 'c' && 
            utext[pos + 2] == '[') {
            pos += 10;
        ...
        // Lecture sécurisée du prochain caractère UTF-8
        if (pos < text_length) {
            UTF8Char ch = {{0}, 0};
            unsigned char c = utext[pos];
            
            if (c < 0x80 && pos < text_length) {  // ASCII standard
                ch.bytes[0] = c;
                ch.length = 1;
            }
            else if ((c & 0xE0) == 0xC0 && pos + 1 < text_length) {  // 2 bytes
                ch.bytes[0] = c;
                ch.bytes[1] = utext[pos + 1];
                ch.length = 2;
            }
            else if ((c & 0xF0) == 0xE0 && pos + 2 < text_length) {  // 3 bytes
                ch.bytes[0] = c;
                ch.bytes[1] = utext[pos + 1];
                ch.bytes[2] = utext[pos + 2];
                ch.length = 3;
            }
            
            pos += (ch.length > 0) ? ch.length : 1;
            current_line_length++;
        } else {
            break;
        }
    ...
    // Mise à jour finale de max_chars_per_line
    max_chars_per_line = (current_line_length > max_chars_per_line) ? 
                        current_line_length : max_chars_per_line;
    ...
    tex = create_alpha_texture(tex_width, tex_height);
    if (!tex) return NULL;
    ...
    // Rendu du texte avec les mêmes vérifications de sécurité
    pos = 0;
    int char_pos = 0;
    int current_line = 0;
    current_color = 0xFFFFFF;
    ...
    while (pos < text_length && utext[pos] != 0) {
        ...
        if(char_index >= 0 && char_pos < max_chars_per_line) {
            ...
            uint8_t g = (current_color >> 8) & 0xFF;
            uint8_t b = current_color & 0xFF;
            color4444 = (0xF << 12) |      // Alpha opaque
                                ((r >> 4) << 8) |    // Rouge
                                ((g >> 4) << 4) |    // Vert
                                (b >> 4);            // Bleu
            //printf("currentColor color4444:%x \n",color4444);
            // Position de base dans le buffer pour ce caractère
            int base_x = char_pos * 8;
            int base_y = current_line * 8;
            
            // Dessiner le caractère avec vérification des limites
            for(int y = 0; y < 8; y++) {
                if(base_y + y >= content_height) break;
                ...
                for(int x = 0; x < 8; x++) {
                    ...
                    int bitmap_idx = (char_index * 64) + (y * 8) + x;
                    if(bitmap_idx >= 0 && font->bitmap && font->bitmap[bitmap_idx]) {
                        int buffer_idx = (base_y + y) * tex_width + (base_x + x);
                        if(buffer_idx >= 0 && buffer_idx < (tex_width * tex_height)) {
    ...
    //printf("Taille final de la texture width:%d height:%d\n", tex_width, tex_height);
    printf("Creating texture %dx%d for content %dx%d color4444: %x bg_color4444: %x \n", 
           tex_width, tex_height, content_width, content_height, color4444,bg_color4444);
